...
with open("aa.pdf", "rb") as input:
    ...
    title_list = ['≪専門必修科目 (必修科目 )≫', '≪専門選択科目 (選択)≫', '≪基礎教育科目 (他学部)≫',
                  '≪基礎教育科目 (実践・教養・心 )≫', '≪導入科目 (導入科目 )≫', '≪英語(英語)≫', '≪外国語科目 (外国語)≫']
    index_list = [0]
    i_1 = allSub_list.index(title_list[0])
    index_list.append(allSub_list.index(title_list[1]))
    index_list.append(allSub_list.index(title_list[2]))
    index_list.append(allSub_list.index(title_list[3]))
    index_list.append(allSub_list.index(title_list[4]))
    index_list.append(allSub_list.index(title_list[5])) 
    index_list.append(allSub_list.index(title_list[6]))            
    #print(index_list)

    '''search_value_2 = "アルゴリズム論"
    doubleSub_index = [i for i, x in enumerate(allSub_list) if search_value_2 in x]
    #print(doubleSub_index)'''
    ...
    #print(allSub_list)
    search_value = "≪"
    title_index_list = [i for i, x in enumerate(allSub_list) if search_value in x]
    ...
    def data_list_cre(title, sub_list):
        if title in allSub_list:
            ...
            hi_i = title_index_list.index(title_index)
            next_index = 0
            if hi_i+1 != len(title_index_list):
                next_index = title_index_list[hi_i + 1]
            else :
